...
def submit(plan_contents: str, github_vars: dict):
    response = requests.post(
        f'{external_service_url}/plan',
        json={
            'plan_base64': base64.b64encode(plan_contents.encode('utf8')).decode('utf8'),
            'github_vars': base64.b64encode(json.dumps(github_vars).encode('utf8')).decode('utf8'),
        },
    )

    if response.status_code != 201:
        print(
            f'Failed submitting plan. External service sent response code {response.status_code}.')
        sys.exit(1)
    ...
    print(
        f'::set-output name=approval_prompt_url::{external_service_url}/plan/{plan_id}')


def wait(plan_id: str, timeout_seconds: int, polling_period_seconds: int):
    print(
        f'Waiting up to {timeout_seconds} seconds for {external_service_url}/plan/{plan_id} to be approved or rejected')
    waited = 0
    while waited <= timeout_seconds:
        response = requests.get(
            f'{external_service_url}/plan/{plan_id}/status')
        if response.status_code != 200:
            print(
                f'Failed polling plan status. External service sent response code {response.status_code}.')
            sys.exit(1)
...
if __name__ == '__main__':
    ...
    env_vars = os.environ
    github_vars = {key: val for key, val in env_vars.items() if key.startswith("GITHUB")}
    print("github_vars is " + str(github_vars))
    ...
    if command == 'submit':
        ...
        submit(plan_contents, github_vars)
    elif command == 'wait':
        ...
        try:
            ...
            print(
                '`timeout_seconds` could not be parsed as integer. Falling back on 300.')
            timeout_seconds = 300

        try:
            polling_period_seconds = int(
                os.getenv('INPUT_POLLING_PERIOD_SECONDS'))
        except:
            print(
                '`polling_period_seconds` could not be parsed as integer. Falling back on 5.')
            polling_period_seconds = 5

        wait(plan_id=plan_id, timeout_seconds=timeout_seconds,
             polling_period_seconds=polling_period_seconds)
    else:
        print(
            f'Unrecognized command: "{command}". Should be one of `submit` or `wait`.')
        sys.exit(1)
